#java #learning_reksy #git


`pwd` - где находишься
`ls` - список в директории
`cd` - перемещаемся по конкретным dir
`..` - путешествуем по дир
`touch` - создает файлы с раширением
`cp` - копирует
`mkdir` - создает папку
`rm` -удаляет файл
`rmdir` - удаляет папку
`cat` - открывает txt файл
`rm - r` - удаление со всем включительным
`&&` используем для связки консольных команд 
https://forge.tcsbank.ru/case/17638028/history заполмнить
\
# Взято с сайта
